...
import postgressConnection from "../../databases/connection";

async function registerUser(req, res) {
  const { username, email, password, organization, isOwner, roles } = req.body;
  let saltRounds = 10;
  ...
  try {
    await postgressConnection.transaction(async (t) => {
      ...
      res.status(200).json({ message: `Account Created`, data: createdUsers });
      return;
    });
  } catch (error) {
    console.log(error);
...
async function loginUser(req, res) {
  ...
  const generatAccessToken = (userData) => {
    return jwt.sign(userData, config.jwtSecretToken, { expiresIn: "86400s" });
  };
